...
// Problem Statement
//     
// Elly is going to take part in a programming contest. Before the contest,
// each contestant is assigned a room (just like in this SRM).  The contest
// uses the following room assignment algorithm:
// Let the number of registrants be N.
// The number of rooms is set to be R = N / 20, if N is divisible by 20, or
// R = N / 20 + 1 otherwise. Here the '/' operator represents integer division,
// i.e., the remainder is being ignored.
// The registrants are sorted by rating in decreasing order.
// The first R registrants are randomly assigned to rooms such that no two of
// them are in the same room. Then the same is done with the next R registrants
// and so on, until there are no more unassigned registrants. Note that it is
// possible for the last group to have less than R registrants, but they are
// still randomly assigned into rooms in such a way that no two of them are in
// the same room.
// You are given a vector <string> ratings. Concatenate its elements to obtain
// one long string. It will represent a space-separated list of the contestants'
// ratings. For simplicity, no two ratings in the list will be equal. Elly's
// rating is the first one in the list.  Return the probability that she will
// be assigned to the room to which the person with the highest rating is
// assigned.
// Definition
//     
// Class:
// EllysRoomAssignmentsDiv2
// Method:
// getProbability
// Parameters:
// vector <string>
...
// Each number in the concatenation of ratings will be an integer without
// leading zeroes.
// Each number in the concatenation of ratings will be a unique integer between
// 1 and 1199, inclusive.
// Every two numbers in the concatenation of ratings will be separated by a
// single space.
// The count of numbers in the concatenation of ratings will be between 8 and
// 652, inclusive.
...
// There are 43 registrants, one of them is Elly. Her raiting is 724. The
// contestants will be distributed into (43/20) + 1 = 3 rooms. One of the rooms
// will have 15 contestants and each of the other two will have 14.
// 1)
...
// Returns: 0.0
// There are five rooms in this case. Being the third highest rated, the room
// assignment algorithm will never put her in the same room with the person
...
public:
    double getProbability(vector <string> ratings)
    {
        ...
        bool first = true;
        string input;
        for (vector<string>::iterator iter = ratings.begin(); iter != ratings.end(); ++iter) {
            input += *iter;
        }
        ...
        int rooms = p.size() / 20 + (p.size() % 20 == 0 ? 0 : 1);
        //cout << rooms << " " << p.size() << endl;
        if (ellys == p[0] || rooms == 1) return 1.0;
        int index;
        ...
        if (index < rooms) return 0.0;
        double result = 1.0 / rooms;
        return result;
    }
private:
};